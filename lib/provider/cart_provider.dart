...
class CartProvider extends ChangeNotifier {
  ...
  Future<void> uploadToServer(BuildContext context) async {
    if ((await cartRepo.getCartList()).length > 0) {
      for (int index = 0;
          index < (await cartRepo.getCartList()).length;
          index++) {
        await addToCartAPI(
          (await cartRepo.getCartList())[index],
          (success, message) {},
          context,
          (await cartRepo.getCartList())[index].choiceOptions,
          (await cartRepo.getCartList())[index].variationIndexes,
        );
